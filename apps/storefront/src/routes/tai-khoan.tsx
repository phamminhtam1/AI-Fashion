...
function Account() {
  const { user, logout, orders, wishlist, sessionReady } = useStore();
  const [tab, setTab] = useState<"overview" | "orders" | "profile">("overview");

  if (!sessionReady) {
    return <div className="py-28 text-center text-muted-foreground">Đang tải…</div>;
  }
    ...
    <div className="mx-auto max-w-[1200px] px-6 py-12">
      ...
      <div className="mt-10 grid gap-10 md:grid-cols-[220px_1fr]">
        <aside className="flex gap-4 overflow-x-auto border-b border-border pb-3 text-sm md:flex-col md:border-b-0 md:border-r md:pb-0">
          {tabs.map(([k, l]) => (
            <button key={k} type="button" onClick={() => setTab(k)} className={`whitespace-nowrap text-left ${tab === k ? "font-medium underline underline-offset-4" : "text-muted-foreground hover:text-foreground"}`}>{l}</button>
          ))}
          <Link to="/yeu-thich" className="whitespace-nowrap text-muted-foreground hover:text-foreground">Yêu thích ({wishlist.length})</Link>
          <button type="button" onClick={() => void logout()} className="whitespace-nowrap text-left text-muted-foreground hover:text-foreground">Đăng xuất</button>
        </aside>

        <section>
          {tab === "overview" && (
            <div className="grid gap-4 sm:grid-cols-3">
              {([["Đơn hàng", orders.length], ["Yêu thích", wishlist.length], ["Hạng thành viên", "Silver"]] as const).map(([l, v]) => (
          ...
          {tab === "profile" && (
            <dl className="divide-y divide-border border-y border-border text-sm">
              {([["Họ và tên", user.name], ["Email", user.email], ["Số điện thoại", user.phone || "—"]] as const).map(([k, v]) => (
